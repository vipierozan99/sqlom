...
| `row_access.txt` | **Positional vs key row access, per driver.** Answers "which is faster and can positional always be used": positional wins by 1.97x (asyncpg `Record`) to 4.76x (`sqlite3.Row`), building a dict per row costs 5-7x and dominates either, and rowform is always positional because it owns the `SELECT` list — there is no `SELECT *` in the builder. Also records one micro-optimization deliberately declined. |
| `sqlite_join.txt` | **Selecting two models across a join**, rowform vs Core vs ORM, isolated processes at 100 and 1000 rows. Shows the ORM advantage largely surviving a join (4.05-4.71x) while the Core advantage shrinks to 1.17-1.32x, plus a GC-on/GC-off matrix proving the whole tail is garbage collection in every contender. |
...
| `isolated_pinned_raw.txt` | rowform and the two hand-written asyncpg baselines, **isolated** (one process each), 3 trials, client 2 cores. |
| `isolated_pinned_sqlalchemy.txt` | SQLAlchemy async Core and ORM, isolated, 3 trials, client 2 cores. |
| `isolated_unpinned.txt` | rowform, async ORM and the codegen-dict floor, isolated, 3 trials, 4 cores shared. |
| `core_sweep_1core_client.txt` | **The headline measurement.** Client pinned to one core, Postgres given 1/2/3 cores, rowform vs async ORM, 3 trials each. |
| `profile_pg_1core.txt` | **Profiled run**: client pinned to core 0, Postgres to cores 2,3. cProfile (CPU timer) + pyinstrument sampling for rowform and the async ORM, plus a `sslmode=disable` contrast run. |
| `profile_sqlite.txt` | **Profiled sqlite run** — no event loop, pool or TLS. cProfile + pyinstrument for rowform, SQLAlchemy Core and ORM, single-threaded on core 0. Isolates the mapper's own cost. |
...
| `psycopg_end_to_end.txt` | The same through FastAPI + uvicorn, with a `/noop` framework floor. **Re-run**: three independent re-runs side by side with the published numbers, including one that had to be discarded and why. |
| `tests.txt` | Output of the pytest suite (698 tests, including mypy and pyright over tests/typing/): SQL generation, codegen, CTEs, upserts, multi-table DML, joins end-to-end on sqlite, and — against live Postgres, parameterised over both engines — lifecycle, `fetch_all`/`fetch_json`, joins, `ON CONFLICT`, recursive CTEs, `UPDATE ... FROM`, `DELETE ... USING`, transactions, savepoints, isolation, the in-transaction guard, and the conditional-reset invariant. Replaces the hand-rolled `verify_transactions.py`. |
| `hold_time.txt` | Whether rowform releasing its pooled connection *before* hydrating (SQLAlchemy holds it through row shaping) inflates the comparison. It does not — making Core symmetric measures 3% slower, and starving the pool to 2 against c=8 moves it by 1 rps, because the client is CPU-bound not connection-bound. |
...
| `final_comparison.txt` | **The bottom line.** rowform vs SQLAlchemy Core/ORM, both fully tuned, data layer only, one core, c=8. |
| `fastapi_end_to_end.txt` | The same comparison through FastAPI + uvicorn, server and load generator on separate cores, including a `/noop` framework floor. **Re-run** after the query builder grew joins through multi-table DML: two independent re-runs with per-trial spread, checked against the published figures rather than replacing them. |
...
| `multiprocess_scaling.txt` | 1 vs 2 rowform worker processes, one core each, Postgres on cores 2,3. |
