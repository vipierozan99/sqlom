...
| file | what it is |
|---|---|
| `core_idiom.txt` | **The largest correction in this suite.** Every published "vs SQLAlchemy Core" ratio was inflated 1.6-2.6x because the harness shaped Core's rows through `.mappings()`, whose `quoted_name` keys force a per-key `str()` cast that orjson requires and sqlom never pays. Measured in all five configurations, plus how the join benchmark exposed it. The ORM ratios are unaffected. Read this before quoting any Core figure. |
| `row_access.txt` | **Positional vs key row access, per driver.** Answers "which is faster and can positional always be used": positional wins by 1.97x (asyncpg `Record`) to 4.76x (`sqlite3.Row`), building a dict per row costs 5-7x and dominates either, and sqlom is always positional because it owns the `SELECT` list — there is no `SELECT *` in the builder. Also records one micro-optimization deliberately declined. |
| `sqlite_join.txt` | **Selecting two models across a join**, sqlom vs Core vs ORM, isolated processes at 100 and 1000 rows. Shows the ORM advantage largely surviving a join (4.05-4.71x) while the Core advantage shrinks to 1.17-1.32x, plus a GC-on/GC-off matrix proving the whole tail is garbage collection in every contender. |
