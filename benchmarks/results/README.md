...
| `transactions.txt` | Transaction semantics verified against live Postgres on both engines — 31 checks: commit, rollback, read-your-writes, invisibility to other connections, savepoint depth, the in-transaction guard, and the conditional-reset invariant (a `SET` inside a block must not leak). Plus the guard's measured cost on the read path (+0.11%). |
