...
async def sqlite_flat_rowform(limit: int = Query(default=DEFAULT_LIMIT)) -> Response:
    ...
    return Response(content=orjson.dumps(rows, default=rf.json_default), media_type=JSON)
...
async def sqlite_join_rowform(limit: int = Query(default=DEFAULT_LIMIT)) -> Response:
    ...
    return Response(content=orjson.dumps(payload, default=rf.json_default), media_type=JSON)
