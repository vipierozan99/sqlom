...
class Condition:
    ...
    def to_sql(self, placeholder="?"):
        ...
        `params` is a tuple rather than a single value because a predicate can
        bind *no* parameter: `User.email == None` must render `email IS NULL`,
        not `email = $1` with a NULL bound, which matches nothing.
        """
        if self.value is None and self.op in _NULL_FORMS:
            return f"{self.column_name} {_NULL_FORMS[self.op]}", ()
