...
rowform) — extending tests/test_expressions.py and tests/test_grouping.py.

Adapted to rowform. Skipped:
...
* Everything in test_lateral.py — every test in that file exercises a LATERAL
  ...
  subquery), and rowform has no LATERAL support at all (grepped rowform/query.py
  and rowform/expr.py for "lateral": no hits). There was no window-function
...
* Most of test_labels.py — it is almost entirely about SQLAlchemy's
  ...
  rowform has none of that: it never invents a label, requires an explicit
...
* test_case_statement.py's dict-based `case({cond: val, ...})` form,
  ...
  rowform has no raw-text expression type, and aggregates/case expressions
...
class TestLabelEdgeCases:
    ...
    def test_label_collision_renders_both_labels_verbatim(self):
        ...
        # (LABEL_STYLE_DISAMBIGUATE_ONLY), rowform does not invent names at
