...
`text()` as a raw SQL fragment, adapted to rowform.

rowform's `text()` differs from SQLAlchemy's in one structural way worth
...
`_result_columns`, etc.) — rowform's is deliberately much smaller: a
...
* `test_text_adds_to_result_map` — rowform has no `_result_columns`/type-map
  machinery to inspect.
* `test_select_composition_one`/`_two`'s `select_from(text(...))` form —
  rowform's `select_from()` (see test_select_ported.py) takes models/aliases,
...
* `test_typing_construction`/the `type_=`/`checkparams`/`literal_binds=`
  machinery — rowform's `bindparams()` takes plain values, no type objects,
  and always binds as a parameter (never inlines a literal into the SQL
  text) — see cast()'s docstring for why rowform has no type-object system.
...
from rowform import Query, text
...
def test_select_composition_with_add_columns_and_two_text_where_clauses():
    ...
    # unlike SQLAlchemy, rowform always needs a real FROM table (there is no
    ...
    from rowform import literal_column
...
# rowform-original test (no SQLAlchemy equivalent) — the one practical case
