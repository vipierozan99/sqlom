...
call afterward, rather than rowform's usual "every value binds immediately at
build time" (see `BindParameter`'s docstring in rowform/expr.py).
...
* `test_bindparam_detection`'s exhaustive escaping/edge-case matrix
  ...
  considerably more conservative than rowform's (a `:` not itself preceded by
  another `:`, full stop). rowform's is deliberately the simpler rule stated
...
entirely different execution model) — those are rowform-original tests,
...
class TestDeferredRendering:
    """rowform-original tests (no SQLAlchemy equivalent) — SQLAlchemy has no
    ...
    compiled string/params tuple the way rowform's does."""
...
class TestCachingIsGenuinelyOnce:
    """rowform-original tests (no SQLAlchemy equivalent) — proving the actual
...
class TestInsertRowValuesFix:
    """rowform-original test (no SQLAlchemy equivalent) — regression test for
