...
class TestEngineStateError:
    ...
    @pytest.mark.parametrize("method", ["fetch_all", "fetch_one", "fetch_value"])
    async def test_it_names_the_method_that_was_called(self, engine, method):
        ...
        caller reached for. `fetch_one` and `fetch_value` routed through
        `fetch_all` and inherited its name, telling the caller to fix a call they
        had not made."""
        async with engine.begin():
